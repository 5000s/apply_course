...
class CourseApplyController extends Controller
{
    ...
    public function directConfirm(Request $request, $course_id, $member_id)
    {
        ...
        if ($full_form && !$no_update) {
            ...
            $member->email = trim($request->input('email')) != "" ? $request->input('email') : $member->email;
            $member->phone = trim($request->input('phone')) != "" ? $request->input('phone') : $member->phone;
            $member->phone_new = trim($request->input('phone_2')) != "" ? $request->input('phone_2') : $member->phone_new;
            $member->line = trim($request->input('line')) != "" ? $request->input('line') : $member->line;
            $member->nationality = trim($request->input('nationality')) != "" ? $request->input('nationality') : $member->nationality;
            $member->province = trim($request->input('province')) != "" ? $request->input('province') : $member->province;
            $member->country = trim($request->input('country')) != "" ? $request->input('country') : $member->country;
            $member->medical_condition = trim($request->input('disease')) != "" ? $request->input('disease') : $member->medical_condition;
            $member->degree = trim($request->input('degree')) != "" ? $request->input('degree') : $member->degree;
            $member->organization = trim($request->input('organization')) != "" ? $request->input('organization') : $member->organization;
            $member->career = trim($request->input('career')) != "" ? $request->input('career') : $member->career;
            $member->expertise = trim($request->input('expertise')) != "" ? $request->input('expertise') : $member->expertise;
            $member->name_emergency = trim($request->input('name_emergency')) != "" ? $request->input('name_emergency') : $member->name_emergency;
